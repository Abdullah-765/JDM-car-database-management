...
export default function DashboardPage() {
  ...
  const handleSaveCar = async (carData: Partial<Car>) => {
    ...
    try {
      if (carToEdit) {
        // Strip id and created_at to avoid updating reserved columns
        const { id, created_at, ...updateData } = carData;
        const { error } = await supabase.from('cars').update(updateData).eq('id', carToEdit.id)
        if (error) {
          console.error("Update error:", error)
          alert("Could not update vehicle: " + (error.message || "Unknown error"))
        }
      } else {
        // Strip id and created_at — they are auto-generated by the database
        const { id, created_at, ...insertData } = carData as any;
        const { error } = await supabase.from('cars').insert([insertData])
        if (error) {
          console.error("Insert error:", error)
          alert("Could not add vehicle: " + (error.message || "Unknown error"))
        }
      }
    } catch (err: any) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      if (error) {
        ...
        alert("Could not delete vehicle: " + (error.message || "Unknown error"))
      }
    } catch (err: any) {
